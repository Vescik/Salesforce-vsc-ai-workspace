...
- Azure DevOps organization and default project can differ locally. Configure them through `make configure`, `workspace.local.json`, or `ADO_ORG`/`ADO_PROJECT`.
- Azure Wiki repository URL, branch, and vendor directory can differ locally. Configure them in `workspace.local.json`, environment variables, or Makefile arguments.
- The Knowledge Base repo URL is optional and should be configured locally or passed through `KB_REPO`.

## Azure DevOps MCP

Committed examples use the placeholder organization `YOUR_ADO_ORG` and server name `ado-remote-mcp`. Replace the placeholder in local VS Code MCP configuration before running `/fetch-us <WORK_ITEM_ID>`.

Do not store ADO PATs, OAuth tokens, passwords, or employee credentials in `.ai/config/*.json`.

## Azure Wiki

Azure DevOps Wiki draft publication uses a Git repository URL from local config, `AZURE_WIKI_REPO`, or a Makefile argument. The local clone cache defaults to `.ai/vendor/azure-wiki/` and is ignored by Git.
