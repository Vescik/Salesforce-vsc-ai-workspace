...
| `make knowledge-index-yaml` | Index KB notes and emit YAML file map. | No | Yes | Writes JSONL and YAML index artifacts. |
| `make knowledge-graph` | Build the knowledge graph index. | No | Yes | Also refreshes KB YAML and metadata knowledge indexes. |
...
| `make knowledge-validate` | Validate curated KB note schema, age, and output reports. | No | Yes | Local validation only. |
| `make metadata-knowledge-index` | Convert Salesforce metadata into local knowledge cards. | No | Yes | Reads local `force-app` metadata. |
