...
> **Platform:** Windows (PowerShell). VS Code Tasks are also available: Ctrl+Shift+P → Tasks: Run Task
...
PowerShell wrappers under `scripts/` are the only entry points; `make` is not used on this branch. Direct invocations like `python scripts/setup.py` also work.
...
Tasks invoke `scripts/workspace.ps1` so behavior is consistent between terminal and VS Code.
...
- PDF export unavailable: run `.\scripts\workspace.ps1 docs-export-pdf` and follow `docs/workspace/pdf/README.md`.
